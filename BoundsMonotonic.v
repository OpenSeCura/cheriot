...
Lemma countTrailingZerosArray_bound_5 : forall ni (arr: @Expr type (Array ni Bool)),
  (ni < 32)%nat ->
  Zmod.unsigned (evalLetExpr (@countTrailingZerosArray type ni arr 5)) <= Z.of_nat ni.
Proof.
  intros ni arr Hni.
  ...
  assert (H_acc: 0 <= Zmod.unsigned (Zmod.zero : bits 5)) by (rewrite H_zero; lia).
  assert (H_bound: Zmod.unsigned (Zmod.zero : bits 5) + Z.of_nat ni < 32) by (rewrite H_zero; lia).
  pose proof (@countTrailingZerosLoop_bound_5 ni arr ni 0 false Zmod.zero H_acc H_bound) as Hloop.
  rewrite H_zero in Hloop.
  lia.
Qed.
...
Proof.
  ...
  fold ef.
  assert (HE_range: 0 <= ef <= 31) by (unfold ef; destruct (_ <? _); apply bits_ExpSz_range).
  rewrite Zmod.unsigned_slu.
  rewrite (unsigned_app_zero (n:=CapBSz) (m:=AddrSz + 1 - CapBSz)) by solve_lia.
  rewrite Z.shiftl_mul_pow2 by solve_lia.
  eapply Z.le_trans.
  { apply Z.mod_le.
  ...
  - (* pick_b = true: mf = 511, e_b < e_init *)
    ...
    pose proof (bits_ExpSz_range eb_bits) as Heb_range.
    apply roundDown_pick_b_math.
    + lia.
    + apply Z.mod_pos_bound. apply Z.pow_pos_nonneg; lia.
    + match goal with
      | H: context [countLeadingZerosLoop ExpSz ?arr ?p ?b ?z] |- _ =>
          set (clz_bits := evalLetExpr (countLeadingZerosLoop ExpSz arr p b z)) in *
      end.
      pose proof (bits_ExpSz_range clz_bits) as Hclz_range.
      assert (Hclz_23: Zmod.unsigned clz_bits <= 23) by (unfold clz_bits; apply (countLeadingZerosArray_bound_5 (ni:=23%nat)); lia).
      rewrite e_init_24_val in Hpick_b by (apply (countLeadingZerosArray_bound_5 (ni:=23%nat)); lia).
      fold eb_bits in Hpick_b.
      fold clz_bits in Hpick_b.
      eapply (@pow2_ef_le_length_clz (Zmod.unsigned length) (Zmod.unsigned eb_bits) (Zmod.unsigned clz_bits)).
      * lia.
      * lia.
      * apply lt_23_le_22; exact Hpick_b.
      * apply length_ge_pow2_clz.
        -- reflexivity.
        -- apply (@lt_23_bound (Zmod.unsigned eb_bits) (Zmod.unsigned clz_bits)); [ lia | exact Hpick_b ].
  - (* pick_b = false: mf = (length >> e_init) mod 512 *)
    ...
    + apply Z.mod_pos_bound; apply Z.pow_pos_nonneg; [ lia | lia ].
Qed.
...
Proof.
  ...
  change (evalLetExpr (countLeadingZerosLoop ExpSz _ _ _ _)) with clz_bits in |- *.
  ...
  destruct (Zmod.eqb _ _) eqn:Hovf.
  - (* Overflow branch *)
    ...
    + (* clz_bits = 0: ef = 24 *)
      ...
      change (Zmod.unsigned (Zmod.of_Z 32 24 : bits 5)) with 24.
      destruct (_ || _) eqn:Hinc.
      * change (if true then 1 else 0) with 1.
        ...
        -- replace 24 with (23 - Zmod.unsigned clz_bits + 1) by lia.
           ...
           rewrite !e_init_24_val in Hovf, Hlsb by (apply (countLeadingZerosArray_bound_5 (ni:=23%nat)); lia).
        ...
        -- replace 24 with (23 - Zmod.unsigned clz_bits + 1) by lia.
           ...
           ++ change (Zmod.of_Z 32 0) with (Zmod.zero : bits ExpSz) in Hovf.
              ...
              rewrite !Zmod.add_0_l in Hovf.
              rewrite !e_init_24_val in Hovf by (apply (countLeadingZerosArray_bound_5 (ni:=23%nat)); lia).
              pose proof (lastn_1_10_eq_1 Hovf) as H512_raw.
           ...
           ++ change (Zmod.of_Z 32 0) with (Zmod.zero : bits ExpSz) in Hbe.
              ...
              rewrite !e_init_24_val in Hbe by (apply (countLeadingZerosArray_bound_5 (ni:=23%nat)); lia).
    ...
    + (* clz_bits > 0: ef = 24 - clz_bits <= 23 *)
      ...
      { apply Z.ltb_ge. apply Z.eqb_neq in Hclz0. lia. }
      rewrite Hsat.
  ...
  - (* Non-overflow branch *)
    ...
    rewrite !e_init_24_val by (apply (countLeadingZerosArray_bound_5 (ni:=23%nat)); lia).
...
Proof.
  ...
  eapply (@bounds_top_from_base_len (Zmod.unsigned (bounds@%"top")) (Zmod.unsigned (bounds@%"base")) (Zmod.unsigned (bounds@%"length")) (Zmod.unsigned base) (Zmod.unsigned length) (Zmod.unsigned (bounds@%"E"))).
...
Lemma bounds_nowrap_le : forall (base length : bits Xlen) (top_bound : Z),
  Zmod.to_Z (base + length) <= top_bound ->
  Zmod.to_Z base + Zmod.to_Z length < 2^Xlen ->
  Zmod.to_Z base + Zmod.to_Z length <= top_bound.
Proof.
  intros b l top_bound H_top_le Hno_wrap.
  assert (Hsum_mod: Zmod.to_Z (b + l) = (Zmod.to_Z b + Zmod.to_Z l) mod 2^Xlen).
  { unfold Zmod.to_Z. change (@Zmod.Private_to_Z ?m) with (@Zmod.unsigned m). apply Zmod.unsigned_add. }
  assert (Hb_nn: 0 <= Zmod.to_Z b) by (apply (@to_Z_nonneg Xlen); change Xlen with 32; lia).
  assert (Hl_nn: 0 <= Zmod.to_Z l) by (apply (@to_Z_nonneg Xlen); change Xlen with 32; lia).
  rewrite Z.mod_small in Hsum_mod by (change Xlen with 32 in *; change (2^32) with 4294967296 in *; lia).
  lia.
Qed.
...
Proof.
  ...
  assert (H_E_top : Zmod.to_Z (bounds@%"E") <= (Zmod.to_Z (evalExpr (get_ECorrected_from_E (evalExpr (get_E_from_cE (cap@%"cE"))))))
                    /\ Zmod.to_Z base + Zmod.to_Z length <= Zmod.to_Z (ecap@%"top")).
  ...
  destruct H_E_top as [HE H_top_sum_le].
  ...
  assert (H_top_math: Zmod.to_Z (bounds@%"top") <= ((Zmod.to_Z base + Zmod.to_Z length + 2^(Zmod.to_Z (bounds@%"E")) - 1) / 2^(Zmod.to_Z (bounds@%"E"))) * 2^(Zmod.to_Z (bounds@%"E"))).
