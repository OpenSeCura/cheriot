...
Definition fifoCount {dom : string} {capacity : nat} {k : Kind} {ty : Kind -> Type} :
  Action ty (fifoTree dom capacity k) (Bit (Z.log2_up (Z.of_nat (capacity + 1)))) :=
  RegRead sz <- "fifo.size" in (fifoTree dom capacity k) ;
  Return #sz.
...
Section SifiveUart.
  ...
  Definition sifiveUartChildren : list (Tree DomainElem) :=
    [ Node "tx" [ fifoTree dom SifiveUartFifoCapacity (Bit 8) ] ;
      Node "rx" [ fifoTree dom SifiveUartFifoCapacity (Bit 8) ] ;
      Leaf "txctrl_txen"  (dom, EReg (Build_Reg Bool (Some false) false)) ;
      Leaf "txctrl_nstop" (dom, EReg (Build_Reg Bool (Some false) false)) ;
      Leaf "txctrl_txcnt" (dom, EReg (Build_Reg (Bit SifiveUartWatermarkWidth) (Some Zmod.zero) false)) ;
      Leaf "rxctrl_rxen"  (dom, EReg (Build_Reg Bool (Some false) false)) ;
      Leaf "rxctrl_rxcnt" (dom, EReg (Build_Reg (Bit SifiveUartWatermarkWidth) (Some Zmod.zero) false)) ;
      Leaf "ie_txwm"      (dom, EReg (Build_Reg Bool (Some false) false)) ;
      Leaf "ie_rxwm"      (dom, EReg (Build_Reg Bool (Some false) false)) ;
      Leaf "ip_txwm"      (dom, EReg (Build_Reg Bool (Some false) false)) ;
      Leaf "ip_rxwm"      (dom, EReg (Build_Reg Bool (Some false) false)) ;
      Leaf "irq"          (dom, EReg (Build_Reg Bool (Some false) true)) ;
      Leaf "div"          (dom, EReg (Build_Reg (Bit 16) (Some Zmod.zero) false)) ;
      Leaf "txData"       (dom, ESend (Bit 8)) ;
      Leaf "txRdy"        (dom, ERecv Bool) ;
      Leaf "rxData"       (dom, ERecv (Option (Bit 8))) ;
      Leaf "rxRdy"        (dom, ESend (Bit 0)) ;
      Leaf "divOut"       (dom, ESend (Bit 16))
    ].

  Definition sifiveUartTree : Tree DomainElem :=
  ...
  Definition sifiveUartIrqPath         : RegPath sifiveUartTree := getChildRegPathTree sifiveUartTree "irq".
  ...
  Definition liftActionEq {ty : Kind -> Type} {t : Tree DomainElem} {k : Kind}
    (p : NodePath t) {t' : Tree DomainElem} (H : getNode p = t') (a : Action ty t' k) : Action ty t k :=
  ...
  Section SifiveUartOperations.
    ...
    Definition readTxData : Action ty sifiveUartTree (Bit Xlen) :=
      LetA txFull : Bool <- liftAction np_tx_fifo (@isFull dom SifiveUartFifoCapacity (Bit 8) ty) ;
    ...
    Definition readRxData : Action ty sifiveUartTree (Bit Xlen) :=
      LetA rxEmpty : Bool <- liftAction np_rx_fifo (@isEmpty dom SifiveUartFifoCapacity (Bit 8) ty) ;
      LetIf rxByte : Bit 8 <-
        If (Not #rxEmpty) Then (
    ...
    Definition sifiveUartIpStep : Action ty sifiveUartTree (Bit 0) :=
      LetA txCount : Bit SifiveUartFifoCountWidth <- liftAction np_tx_fifo (@fifoCount dom SifiveUartFifoCapacity (Bit 8) ty) ;
    ...
    (* SiFive UART Interrupt Rule: writes interrupt status into cross register *)
    Definition sifiveUartIrq : Action ty sifiveUartTree (Bit 0) :=
      LetA txwm_ip : Bool <- ReadReg "ip_txwm" sifiveUartIpTxwmPath (fun v => Return #v) ;
      LetA rxwm_ip : Bool <- ReadReg "ip_rxwm" sifiveUartIpRxwmPath (fun v => Return #v) ;
      ...
      WriteReg sifiveUartIrqPath #any_irq Retv.

    (* SiFive UART Interrupt Output: reads cross register for PLIC connection *)
    Definition readSifiveUartIrq : Action ty sifiveUartTree Bool :=
      ReadReg "irq" sifiveUartIrqPath (fun v => Return #v).
        ...
        If (Not #txEmpty) Then (
          ...
          If #hasData Then (
            ...
            Recv "txRdy" sifiveUartTxRdyPath (fun txRdy =>
              If #txRdy Then (
                Act (liftAction np_tx_fifo (@deq dom SifiveUartFifoCapacity (Bit 8) ty)) ;
                Retv
              ) ;
              Retv
            )
    ...
    Definition sifiveUartRxStep : Action ty sifiveUartTree (Bit 0) :=
      ...
      If #rxen Then (
        LetA rxFull : Bool <- liftAction np_rx_fifo (@isFull dom SifiveUartFifoCapacity (Bit 8) ty) ;
        If (Not #rxFull) Then (
  ...
  Definition sifiveUartLineWriteAction
    ...
    If (Eq #regIdx (sifiveUartRegIdxBit "txdata")) Then (
      LetA txFull : Bool <- liftAction np_tx_fifo (@isFull dom SifiveUartFifoCapacity (Bit 8) ty) ;
      If (Not #txFull) Then (
        liftAction np_tx_fifo (@enq dom SifiveUartFifoCapacity (Bit 8) ty dataByte)
      ) ;
      Retv
    ) ;
  ...
  Definition sifiveUartMemRegion
    ...
    regionKind        := @CustomMem "sifive_uart" SifiveUartSizeBytes SifiveUartLineConfig
                                    ...
                                    (Some (fun ty => readSifiveUartIrq (ty:=ty))) ;
  ...
  Section SifiveUartSystem.
    ...
    Definition sifiveUartIrqStepAction : Action ty memTree (Bit 0) :=
      sifiveUartAction (@sifiveUartIrq ty).
